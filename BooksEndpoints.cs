...
namespace LibraTrack
{
    public class BooksEndpoints
    {
        public static void AddBookApi(RouteGroupBuilder booksGroup)
        {
            ...
            {
                var book = await dbContext.Books.FirstOrDefaultAsync(b => b.Id == bookId && b.Section.Id == sectionId && b.Section.Library.Id == libraryId);
                if (book == null)
            ...
            {
                var section = await dbContext.Sections.FirstOrDefaultAsync(sec => sec.Id == sectionId);
                if (section == null)
                {
                    return Results.NotFound();
                }
